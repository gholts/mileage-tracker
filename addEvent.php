...
var hideDescription = function()
{
	...
	if(selectedOption == 18)
	{
		...
		document.getElementById("routeHide").style.visibility = "hidden";
	}
	else
	{
		document.getElementById("routeHide").style.visibility = "visible";
		document.getElementById("routeDescription").value = arrayRoutes[selectedOption];
		document.getElementById("routeDescription").readOnly = true;
		//document.getElementById("description").value = arrayRoutes[selectedOption];
		document.getElementById("description").readOnly = false;
	}
}
...
<form name="evFrm" id="evFrm" action=<?php echo "Edit.php?eventid=$row[0]"; ?> method="POST" >
...
Which route are you taking: <select id='selRoute' name='selRoute' onchange='hideDescription()' ><?php addRoutes($row[4]) ?></select>
<div id="routeHide">Route description: <input type='text' id='routeDescription' name='routeDescription' style='width: 200px; resize:none;' /> <br/></div>
Enter the events description: <br/><textarea id='description' value='<?php echo $row[3];?>' name='description' style='height: 100px; width: 300px; resize:none;' ><?php echo $row[3];?></textarea> <br/>
...
?>
<h3>Enter Event to the Calendar</h3>
<form name="evFrm" id="evFrm" action="submit.php" method="POST" >
Enter the event name: <input type='text' id='eventName' name='eventName' /> <br />
 <?php echo "Event date (YYYY-MM-DD): $tDate"?><br />
Which route are you taking: <select id='selRoute' name='selRoute' onchange='hideDescription()'><?php addRoutes(0); ?></select><br />
<div id="routeHide" >Route description: <input type='text' id='routeDescription' name='routeDescription' style='width: 200px; resize:none;' /> <br/></div>
...
function addRoutes($routeID)
{
	...
	while($row = $qryResults->fetch_assoc())
	{
		...
		{
			echo "<option selected='selected' value=".$row['routeID'].">".$row['routeName']."</option>";
		}
		else
		{
	...
	if($routeID == 0 || $routeID == 18)
	{
		echo "<option selected='selected' value=18>Non-Ride</option>";
		
		echo "document.getElementById('routeHide').style.visibility = 'hidden'";
		
		
	}
	else
	{
		echo "<option value=18>Non-Ride</option>";
	}
	disconnect($link);
}
